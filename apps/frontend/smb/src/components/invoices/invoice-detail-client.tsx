            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">80% of invoice value</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {invoice.payer.industry && (
                      <p className="text-xs text-muted-foreground">{invoice.payer.industry}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Badge variant={invoice.riskScore === 'LOW' ? 'default' : 'secondary'}>
                    {invoice.riskScore}
                  </Badge>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">APY</p>
                  <p className="text-lg font-semibold text-green-600">{invoice.apy}%</p>
                </div>
                <div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold">
                      {formatCurrency(invoice.fundedAmount)} / {formatCurrency(invoice.amount)}
                    </span>
                  </div>
                  <Progress value={invoice.fundingProgress} className="h-2" />
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">Progress</span>
                    <span className="font-semibold">{invoice.fundingProgress}%</span>
                  </div>
                </div>
                ...
                {invoice.status === InvoiceStatus.FULLY_FUNDED && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-green-700 mt-1">
                      Your invoice has been fully funded. Funds will be disbursed to your account
                      shortly.
                    </p>
                  </div>
                )}
                ...
                {invoice.status === InvoiceStatus.PARTIALLY_FUNDED && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-blue-700 mt-1">
                      Your invoice is being funded by investors. You'll receive funds once it
                      reaches 100%.
                    </p>
                  </div>
                )}
                ...
                {invoice.status === InvoiceStatus.LISTED && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-yellow-700 mt-1">
                      Your invoice is now available to investors. Funding typically completes
                      within 24 hours.
                    </p>
                  </div>
                )}
              ...
              {invoice.disbursedDate && (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{formatDate(invoice.disbursedDate)}</p>
                  </div>
                </>
              )}
              {invoice.repaidDate && (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{formatDate(invoice.repaidDate)}</p>
                  </div>
                </>
              )}
            </CardContent>
